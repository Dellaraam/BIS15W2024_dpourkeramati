...
We've been processing structured and unstructured data with the msleep and penguins packages
...
over20 <- elephants %>%
  group_by(sex) %>%
  filter(age>=20) %>%
  summarize(ave_height=mean(height),mini=min(height),big=max(height), n=n())
over20
```
...
```
```{r}
cats$HuntCat <- as.factor(cats$HuntCat)
cats$LandUse <- as.factor(cats$LandUse)
```
...
cats %>%
  ...
  summarize(ave_div_birds=mean(Diversity_BirdSpecies),ave_div_mammals=mean(Diversity_MammalSpecies))
```
```{r}
cats %>%
  filter(HuntCat=="High") %>%
