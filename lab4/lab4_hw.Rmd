...
```{r}
anyNA(herbi$mean.hra.m2)
hmean <- mean(herbi$mean.hra.m2)
hmean
```

```{r}
cmean <- mean(carni$mean.hra.m2)
cmean
```
...
```{r}
homerange %>%
  select(mean.mass.g, log10.mass, family, genus, species, order, common.name) %>%  
...
```
Cheriway is found in Central and South America. It mainly feeds on the bodies of dead animals. 
