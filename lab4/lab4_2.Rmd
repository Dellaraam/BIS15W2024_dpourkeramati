...
```{r}
filter(fish,between(scalelength,0.25,0.8))
```
...
```{r}
mammals_new <- select(mammals,family,genus="Genus",species,mass,gestation)
mammals_new
```
...
```{r}
mammals %>%
  select(mass,gestation,newborn,Genus) %>%
  filter(Genus=="Lophocbus" | Genus=="Erythrobus" | Genus=="Macaca") %>%
  arrange(newborn)
```
Macaca
