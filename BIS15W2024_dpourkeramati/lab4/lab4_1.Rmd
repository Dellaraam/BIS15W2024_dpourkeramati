...
```{r}
select(fish, ends_with("id")) #handy bit of code for the project
```
...
```{r}
mean(mammals$`max. life` >= 0)
```

```{r}
select(mammals,`max. life`)
```
...
```{r}
renames <- rename(mammals, genus="Genus", wean_mass="wean mass", max_life="max. life", litter_size="litter size", litters_per_year="litters/year")
renames
```
...
```{r}
mass <- select(renames, "genus", "species", "mass")
mass
```
