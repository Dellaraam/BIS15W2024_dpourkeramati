...
msleep %>%
  ...
  summarize(mean_bodywt = mean(bodywt),min_bodywt=min(bodywt),max_bodywt=max(bodywt), n=n())
```
...
msleep %>% 
  filter(bodywt > 100) %>%
  summarize(distinct=n_distinct(genus))
```
...
n_distinct(msleep$name)
```
```{r}
msleep %>%
  select(sleep_total) %>%
  summarize(mean_sleep=mean(sleep_total),max_sleep=max(sleep_total),min_sleep=min(sleep_total),n=n())
```
