...
deserts %>%
  ...
  labs(title="Number of Samples for Species DM & DS", x="Year", y="n")
```
...
homerange %>% 
  ggplot(aes(x = log10.mass)) +
  geom_histogram(color = "black", fill = "azure4", bins=10)+
  labs(title = "Distribution of Body Mass")
```
...
homerange %>%
  ...
  geom_histogram(color = "pink", fill="pink4")
...
homerange %>%
  ggplot(aes(x=log10.hra))+
  geom_density(color = "pink", alpha=0.2,fill="pink4")+
...
homerange %>%
  ggplot(aes(x=log10.hra))+
  geom_histogram(aes(y=after_stat(density)), color = "pink", fill="pink4")
```
...
homerange %>% 
             mutate(mass_category = case_when(log10.mass <= 1.75 ~ "small",
                                              log10.mass > 1.75 & log10.mass <= 2.75 ~
                                              "medium",log10.mass >
                                              2.75 ~ "large"))
```
...
```{r}
homerange %>% 
      mutate(mass_category = case_when(log10.mass <= 3.65 ~ "very small", log10.mass <= 1.75 ~ "small",
                                              log10.mass > 1.75 & log10.mass <= 2.75 ~
                                              "medium",log10.mass >
                                              2.75 ~ "large"))
```
