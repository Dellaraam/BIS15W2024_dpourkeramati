...
ynp %>%
  filter(pack=="druid") %>%
  group_by(biolyr) %>%
  summarize(ave_pack_size=mean(packsize_aug))
```
...
```
The packs alpha female was killed by another wolf pack and the alpha male did not bread with any other female. Several members including the alpha male also had mange which the probably died from.  
...
ynp %>% 
  group_by(pack) %>%
  summarize(overall_pers=sum(persisty1,na.rm=TRUE)) %>%
  arrange(desc(overall_pers))
```
mollies has persisted through out the years listed. They learned to specilaize in the hunting of bison. 
...
wolves %>%
  group_by(biolyr) %>%
  summarize(mort_lead=sum(mort_lead,na.rm=TRUE))
```
