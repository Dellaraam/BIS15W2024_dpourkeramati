...
```{r}
relig_income <- readr::read_csv(file = "data/relig_income.csv", na = c("-999")) %>% clean_names()
relig_income
```
...
```{r}
relig_income %>%
  pivot_longer(-religion, names_to="income", values_to="people")
```
...
```{r}
plant_data <- readr::read_csv("data/plant_data.csv", na = "-999") %>% clean_names()
```
